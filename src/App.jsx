...
function App() {
  ...
  useEffect(() => {
    ...
    const lockedPills = document.querySelectorAll(
      ".class-pills .class-pill.locked",
    );
    const pillHandlers = [];
    ...
    return () => {
      ...
      pillHandlers.forEach(({ p, handler }) =>
  ...
  return (
    <>
      {showBootScreen && (
        <LoadingScreen onComplete={() => setShowBootScreen(false)} />
      )}
      <Header />
          ...
          <Route
            ...
            element={
              <div style={{ padding: "20px" }}>
                Class not found or coming soon.
              </div>
            }
          />
        </Routes>
      ...
      {/* Progress Indicator */}
      <div className={`scroll-progress ${showProgress ? "visible" : ""}`}>
        {scrollProgress}%
      </div>
